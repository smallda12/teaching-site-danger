...
(function () {
  ...
  const 相簿 = (function () {
    ...
    function 開() {
      return new Promise((res) => {
        if (db) return res(db);
        const r = indexedDB.open("克服危險照片", 1);
  ...
  const 點名 = (function () {
    ...
    async function 匯出() {
      ...
      const 資料 = { 格式: "克服危險點名名單", 版本: 1, 匯出時間: new Date().toISOString(), 學生: [] };
      for (const s of 名單) {
  ...
  function 建影片清單() {
    const 播放器 = $("#影片播放器");
    const 清單 = $("#影片清單");
    const 項目 = [{ 圖示: "\uD83C\uDFAC", 標題: "教學影片（完整版）", 檔: UNIT.資源.影片 }]
      .concat(UNIT.資源.新版影片 ? [{ 圖示: "\uD83C\uDD95", 標題: UNIT.資源.新版影片標題 || "教學影片（新版）", 檔: UNIT.資源.新版影片 }] : [])
      .concat(
      (UNIT.重點 || []).flatMap((r) =>
